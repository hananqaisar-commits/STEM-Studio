...
exports.buildGraphCheckpoints = buildGraphCheckpoints;
...
function outgoingEdges(step, id) {
...
function distanceOf(step, id) {
    const value = step.distances?.[id];
    return typeof value === 'number' ? value : Infinity;
}
function inDegreeOf(step, id) {
 ...
 */
function uniqueArgMin(items, score) {
    let best = null;
...
function plural(items, singular, many) {
 ...
 */
function isVertexAdvance(current, next) {
...
function isEdgeEventOn(current, next) {
...
function isQueueShift(current, next) {
    return (current.queueOrStack.length > 0 &&
...
const ANCHORS = {
    dfs: {
        ...
        explanation: 'A stack. DFS pushes each vertex it descends into and pops it on the way back out, so the most recently discovered vertex is always the next one explored. The recursion stack shown under the canvas is that stack made visible.',
    ...
    prim: {
        ...
        explanation: "The lightest edge crossing the cut between the tree and everything outside it. That is the cut property, and it is also what separates Prim's from Kruskal's: Kruskal's takes the globally lightest edge that stays acyclic, which need not touch the tree at all.",
    ...
    topoSort: {
        ...
        explanation: 'In-degree zero means every prerequisite of that vertex has already been emitted, so putting it next cannot violate the direction of any edge. Vertices with a positive in-degree are still waiting on something.',
 ...
 */
function authoredCandidates(steps, category) {
...
function anchorCandidate(steps, category) {
    const anchor = ANCHORS[category];
    if (!anchor || steps.length < 2)
        return [];
    return [
        {
            stepIndex: 0,
...
function dfsCandidates(steps) {
    ...
    for (let index = 0; index < steps.length - 2; index++) {
        ...
        if (descends !== (unexplored.length > 0))
            continue;
        const parent = depth >= 2 ? current.queueOrStack[depth - 2] : null;
        if (!descends && parent === null)
            continue;
        candidates.push({
            stepIndex: index,
            kind: 'dfs-direction',
...
function dijkstraCandidates(steps) {
    const candidates = [];
    for (let index = 0; index < steps.length - 1; index++) {
        const current = steps[index];
        const next = steps[index + 1];
        ...
        if (isVertexAdvance(current, next)) {
            ...
            if (chosen !== null && chosen === next.currentNodeId) {
                ...
                const unreached = current.nodes
                    .map((node) => node.id)
                    .filter((id) => !current.visitedNodeIds.includes(id) &&
...
function primCandidates(steps) {
    ...
    for (let index = 0; index < steps.length - 1; index++) {
        ...
        const grows = next.currentEdgeId !== null &&
            next.currentNodeId !== null &&
            next.visitedNodeIds.length === current.visitedNodeIds.length + 1;
        if (!grows)
            continue;
        const inTree = new Set(current.visitedNodeIds);
        const crossing = current.edges.filter((edge) => inTree.has(edge.from) !== inTree.has(edge.to));
        const chosen = uniqueArgMin(crossing, weightOf);
        if (chosen === null || chosen.id !== next.currentEdgeId)
            continue;
    ...
    for (let index = 0; index < steps.length - 1; index++) {
        ...
        if (isVertexAdvance(current, next) && isQueueShift(current, next)) {
            ...
            const blocked = current.nodes
                .filter((node) => !current.visitedNodeIds.includes(node.id) &&
...
function place(candidates) {
    ...
    return ordered.map((candidate) => {
        ...
        const weight = candidate.fixedWeight ?? (occurrence < candidate.reinforce ? 2 : 3);
        const id = `graph-${candidate.kind}-${candidate.stepIndex}`;
        const built = (0, Quiz_1.buildOptions)(id, candidate.correct, candidate.distractors);
        return {
 ...
 */
function buildGraphCheckpoints(steps, category) {
    if (steps.length < 2)
        return [];
    const derived = (() => {
        switch (category) {
