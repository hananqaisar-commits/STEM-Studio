...
exports.buildSortingCheckpoints = buildSortingCheckpoints;
const Quiz_1 = require("../../engine/types/Quiz");
...
const ANCHORS = {
    bubble: {
        ...
        explanation: 'A pass compares every adjacent pair and swaps whenever they are out of order, which carries the largest value all the way right. That is why exactly one element locks in per pass, and why n-1 passes are enough.',
    ...
    selection: {
        ...
        explanation: 'A pass only scans for the minimum, tracking its index in minIdx. The array is written once, when that minimum is swapped into place. Few writes is selection sort’s one real advantage over bubble sort.',
        hint: 'Scanning for the smallest value does not require moving anything.',
        concept: 'Write cost',
    },
    insertion: {
        prompt: 'Before it starts: what is true of the region left of the current key, but not of the whole array?',
        ...
        explanation: 'Insertion sort grows a sorted prefix one element at a time. The prefix is internally sorted, but an element not reached yet can still be smaller than everything in it, which would shift the whole prefix right.',
        hint: 'A value further right could still be smaller than everything scanned so far.',
        concept: 'Sorted prefix',
    },
    merge: {
        prompt: 'Before it starts: why can two sorted halves be merged in a single left-to-right scan?',
        ...
        explanation: 'Sortedness means each half’s smallest remaining value is its front element, so comparing just those two fronts is enough to choose the next output value. That is what makes a merge O(n) rather than O(n log n).',
    ...
    quick: {
        ...
        explanation: 'Partitioning puts every smaller value left of the pivot and every larger value right of it — which is exactly the pivot’s sorted position. Recursion can then ignore it and sort only the two sides.',
    ...
    heap: {
        ...
        explanation: 'A max-heap only requires every parent to be at least as large as its children, which forces the maximum to the root. Heap sort repeatedly swaps that root to the end and re-heapifies the shrinking prefix.',
    ...
    shell: {
        ...
        explanation: 'Plain insertion sort moves a value one slot per write, so a value far from home is expensive. Large gaps let it jump many positions at once, leaving the final gap-of-1 pass with very little left to do.',
...
const LOCK_IN_COPY = {
    bubble: {
        ...
        explanation: (index) => `Index ${index}. A pass carries the largest unsorted value to the far right of the unsorted region, so the rightmost unsorted position is the one that locks in.`,
        hint: 'Adjacent swaps push large values right. Which end of the unsorted region benefits?',
    },
    selection: {
        prompt: 'This pass is ending. Which position is now guaranteed to hold its final value?',
        explanation: (index) => `Index ${index}. The pass scanned for the smallest remaining value and swapped it into the first unsorted slot, so that slot is done.`,
    ...
    quick: {
        ...
        explanation: (index) => `Index ${index}. The pivot goes directly after the last value smaller than it, so its final index is the range start plus the count of smaller values.`,
    ...
    heap: {
        ...
        explanation: (index) => `Index ${index}. Heap sort swaps the root with the last slot of the current heap, shrinks the heap by one, and re-heapifies — so the largest value lands at the end of the unsorted region.`,
...
const LOCK_IN_NEEDS_SWAP = new Set(['quick', 'heap']);
/* ── Derivation helpers ──────────────────────────────────────────────── */
function arraysEqual(a, b) {
    return a.length === b.length && a.every((value, index) => value === b[index]);
}
function hasSwap(step) {
 ...
 */
function indexOptions(id, correct, candidates, size) {
    const seen = new Set([correct]);
    const kept = [];
    for (const candidate of candidates) {
        if (kept.length === 3)
            break;
        if (candidate < 0 || candidate >= size || seen.has(candidate))
            continue;
        seen.add(candidate);
        kept.push(candidate);
    }
    if (kept.length === 0)
        return null;
    return (0, Quiz_1.buildOptions)(id, indexLabel(correct), kept.map(indexLabel));
}
...
function nearbyIndices(correct, size) {
...
function insertionLanding(steps, from) {
...
function isSelectionStep(step) {
    return (step.pivotIndex !== undefined &&
 ...
 */
function buildSortingCheckpoints(steps, algorithm) {
    if (steps.length < 2)
        return [];
    ...
    const anchorOptions = (0, Quiz_1.buildOptions)(anchorId, anchor.correct, anchor.distractors);
    checkpoints.push({
        stepIndex: 0,
        question: {
            id: anchorId,
    ...
    const lockInSteps = new Set();
    if (kinds.includes('lockIn') && indexQuestionsUsable) {
        for (let index = 1; index < steps.length - 1; index++) {
            const next = steps[index + 1];
            if (LOCK_IN_NEEDS_SWAP.has(algorithm) && !hasSwap(next))
                continue;
            if (newlySorted(steps[index], next) !== null)
                lockInSteps.add(index);
        }
    }
    const counts = {
        compare: 0,
    ...
    for (let index = 1; index < steps.length - 1; index++) {
        ...
        if (lockInSteps.has(index)) {
            ...
            if (locked !== null && copy) {
                ...
                const built = indexOptions(id, locked, nearbyIndices(locked, size).filter((candidate) => !sorted.has(candidate)), size);
                if (built) {
                    checkpoints.push({
                        stepIndex: index,
                        question: {
        ...
        if (kinds.includes('compare')) {
            ...
            if (pair.length === 2 && pair.every((i) => i >= 0 && i < size)) {
                ...
                const built = (0, Quiz_1.buildOptions)(id, willSwap ? 'Swap them' : 'Leave them and move on', [
        ...
        if (kinds.includes('partition')) {
            ...
            if (pair.length === 2 && j !== null && j >= 0 && j < size) {
                ...
                const built = (0, Quiz_1.buildOptions)(id, willMove
        ...
        if (kinds.includes('landing') && indexQuestionsUsable && isSelectionStep(current)) {
            ...
            if (key !== null && from !== undefined && landing !== null && landing !== from) {
                ...
                const built = indexOptions(id, landing, [...nearbyIndices(landing, from + 1), from], from + 1);
                if (built) {
                    checkpoints.push({
                        stepIndex: index,
                        question: {
        ...
        if (kinds.includes('shift') && isSelectionStep(current)) {
            ...
            if (from !== undefined) {
                ...
                const built = (0, Quiz_1.buildOptions)(id, willShift
        ...
        if (kinds.includes('midpoint') && indexQuestionsUsable) {
            ...
            if (range) {
                ...
                const built = indexOptions(id, midpoint, [midpoint + 1, midpoint - 1, range.right, range.left], size);
                if (built) {
                    checkpoints.push({
                        stepIndex: index,
                        question: {
