    ...
    topics: [
      {
        ...
        name: 'Virtual File System (VFS) Architecture',
      ...
      },
      {
        id: 'root-directory',
        name: 'Root Directory ( / )',
        complexity: 'Anchor Node',
        description: 'The top-level parent directory of the entire Linux filesystem hierarchy. Every single file and directory stems from /.',
        keyPoints: [
          'Represented as a single forward slash /.',
          'Owned by root superuser with default 755 permissions.',
          'All mounted partitions and pseudo-filesystems anchor here.',
        ],
        example: 'cd / && ls -la',
        applications: ['Filesystem hierarchy root', 'Absolute path resolution'],
      },
      {
        id: 'dev-directory',
        name: 'Device Nodes ( /dev )',
        complexity: 'Kernel I/O',
        description: 'Contains special device nodes representing hardware devices (block/char) and kernel pseudo-devices.',
        keyPoints: [
          '/dev/sda, /dev/nvme0n1 represent physical storage disks and partitions.',
          '/dev/null discards all data written to it (black hole stream).',
          '/dev/zero provides infinite null bytes stream; /dev/urandom provides random bytes.',
        ],
        example: 'echo "silent output" > /dev/null',
        applications: ['Storage block devices', 'Stream redirection', 'Hardware abstraction'],
      },
      {
        id: 'etc-directory',
        name: 'System Configuration ( /etc )',
        complexity: 'Configuration',
        description: 'Contains system-wide administration configuration files and system databases.',
        keyPoints: [
          '/etc/passwd defines user accounts and login shells.',
          '/etc/group defines system security groups.',
          '/etc/fstab defines filesystem mount options at boot.',
          '/etc/hosts maps static IP hostnames.',
        ],
        example: 'cat /etc/passwd | grep octa',
        applications: ['System administration', 'Security configuration', 'Boot loading'],
      },
      {
        id: 'tmp-directory',
        name: 'Temporary Storage & Sticky Bit ( /tmp )',
        complexity: 'Mode 1777 (Sticky)',
        description: 'System-wide scratchpad for temporary files created by applications and users.',
        keyPoints: [
          'Configured with Sticky Bit permission 1777 (drwxrwxrwt).',
          'Sticky Bit ensures users can read/write but ONLY delete their own files.',
          'Usually purged automatically on system reboot or by tmpfiles.d cleanup services.',
        ],
        example: 'chmod 1777 /tmp && ls -ld /tmp',
        applications: ['Application scratch buffers', 'Multi-user security isolation'],
      },
      {
        id: 'var-directory',
        name: 'Variable Data & Logs ( /var )',
        complexity: 'Dynamic Data',
        description: 'Houses files whose content dynamically expands during normal system operation (logs, caches, spools).',
        keyPoints: [
          '/var/log contains system and daemon log files (e.g. syslog, auth.log).',
          '/var/spool holds print queues and mail queues.',
          '/var/tmp preserves temporary files across system reboots.',
        ],
        example: 'tail -f /var/log/syslog',
        applications: ['System auditing', 'Daemon log tracking', 'Service spooling'],
      },
      {
        id: 'mnt-media-directory',
        name: 'Mount Points ( /mnt & /media )',
        complexity: 'VFS Mount Points',
        description: 'Directories dedicated to mounting external or temporary storage filesystems.',
        keyPoints: [
          '/media is automatically managed by desktop environments for removable USB drives & CDs.',
          '/mnt is reserved for temporary manual filesystem mounts by administrators.',
        ],
        example: 'sudo mount /dev/sdb1 /mnt',
        applications: ['External drive access', 'NFS/SMB network shares'],
      },
    ],
  },
};
