...
import { Bot, Send, Mic, MicOff, Settings, Minimize2, Trash2, Sparkles, Volume2, Cpu } from 'lucide-react';
...
export const OctaTutor: React.FC = () => {
  ...
  // Character-by-character reveal for latest assistant message
      ...
      if (currentLen < latestMsg.content.length) {
        const timer = setTimeout(() => {
          setRevealedChars((prev) => ({
            ...prev,
            [msgId]: currentLen + 3, // fast 3-char chunks for rapid reveal
          }));
        }, 15);
  ...
  const suggestions = [
    ...
    { label: 'Explain current step', text: `Explain step ${contextState.currentStepIndex + 1} of ${contextState.totalSteps || 1}.` },
    { label: 'Switch to dark mode', text: 'Switch to dark mode.' },
    { label: 'Hide debugger', text: 'Hide code debugger.' },
    { label: 'Generate Quiz', text: `Generate a 5-question quiz on ${contextState.algorithmName || 'this algorithm'}.` },
  ];
    ...
    <div className="octa-tutor-window animate-fade-in" role="region" aria-label="Octa AI Tutor Assistant">
      ...
      <div className="tutor-header">
        <div className="tutor-header-title">
          ...
          <div>
            <div className="tutor-header-name" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              Octa AI Tutor
              <span
                style={{
                  fontSize: '0.62rem',
      ...
      <div className="tutor-context-ribbon">
        <span>📍 {contextState.algorithmName || 'General DSA'}</span>
        <span>
          Step {contextState.currentStepIndex + 1} of {contextState.totalSteps || 1}
        </span>
      </div>

      {/* Messages Scroll Output Box */}
      ...
      {/* Input Container with Perched Mascot TOP-RIGHT (Matching User Reference Image) */}
      <div className="tutor-input-container">
        {/* MASCOT SITUATION: PERCHED EXACTLY ON THE TOP-RIGHT EDGE OF INPUT BOX */}
        ...
        <div className="tutor-input-tools">
          <div className="tutor-lang-selector" title="Speech Recognition Language">
        ...
        {/* Textarea Input (Standard native copy/paste/select textarea) */}
