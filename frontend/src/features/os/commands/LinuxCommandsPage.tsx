              ...
              return (
                <div key={group.id} className="space-y-4">
                  ...
                  <div className="flex items-center gap-3 pb-2 border-b border-[var(--color-border)] dark:border-slate-800/80">
                    <div className="p-2 rounded-lg bg-[var(--color-primary)]/10 dark:bg-slate-800/80 text-[var(--color-primary)] dark:text-cyan-400">
                      <GroupIcon size={20} />
                    </div>
                    <div>
                      <h2 className="text-lg font-bold text-[var(--color-text)] dark:text-slate-100">{group.title}</h2>
                  ...
                  <div className="space-y-4">
                    {group.commands.map(cmd => (
                      <div
                        key={cmd.id}
                        className="rounded-2xl bg-[var(--color-surface)] dark:bg-slate-900/60 border border-[var(--color-border)] dark:border-slate-800/90 p-5 shadow-sm space-y-4 transition-all hover:border-[var(--color-border-strong)] dark:hover:border-slate-700/80"
                      >
                        {/* Command Card Header */}
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            <span className="font-mono text-base font-bold text-[var(--color-primary)] dark:text-cyan-400 bg-[var(--color-primary)]/10 dark:bg-cyan-500/10 px-3 py-1 rounded-lg border border-[var(--color-primary)]/20 dark:border-cyan-500/20">
                              {cmd.name}
                            </span>
                            {cmd.badge && (
                              <span className="text-[11px] px-2.5 py-0.5 rounded-full bg-amber-500/15 text-amber-700 dark:text-amber-300 border border-amber-500/30 font-semibold flex items-center gap-1">
                                <Sparkles size={12} /> {cmd.badge}
                              </span>
                            )}
                          </div>
                          {cmd.type === 'concept' && (
                            <span className="text-[11px] px-2.5 py-0.5 rounded-full bg-blue-500/10 text-blue-600 dark:text-blue-400 border border-blue-500/20 font-medium">
                              Core Concept
                            </span>
                          )}
                        </div>
                        ...
                        {/* Theory Block */}
                        <div className="text-sm text-[var(--color-text)] dark:text-slate-300 leading-relaxed bg-[var(--color-surface-elevated)] dark:bg-slate-950/40 p-3.5 rounded-xl border border-[var(--color-border)] dark:border-slate-800/60">
                          <div className="text-xs font-semibold text-[var(--color-text-muted)] dark:text-slate-400 mb-1 flex items-center gap-1.5">
                        ...
                        <div className="space-y-1.5">
                          <div className="text-xs font-semibold text-[var(--color-text-muted)] dark:text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                            <FileText size={14} className="text-emerald-600 dark:text-emerald-400" /> Syntax
                          </div>
                          <div className="font-mono text-xs bg-slate-900 dark:bg-slate-950 text-emerald-400 p-3 rounded-xl border border-slate-800 overflow-x-auto whitespace-pre-wrap">
                            {cmd.syntax}
                          </div>
                        </div>
                          ...
                          <div className="mt-4 p-4 rounded-xl bg-[var(--color-surface-elevated)] dark:bg-slate-950/80 border border-[var(--color-primary)]/30 dark:border-cyan-500/30 space-y-3">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 pt-2">
                              {cmd.vimModes.map((mode, idx) => (
                                <div key={idx} className="p-3 rounded-lg bg-[var(--color-surface)] dark:bg-slate-900 border border-[var(--color-border)] dark:border-slate-800 space-y-2">
                                  ...
                                  <div className="space-y-1 pt-1 border-t border-[var(--color-border)] dark:border-slate-800">
                                    {mode.keybindings.map((kb, kidx) => (
                                      <div key={kidx} className="font-mono text-[10px] text-amber-700 dark:text-amber-300/90 bg-amber-500/10 px-1.5 py-0.5 rounded border border-amber-500/20 truncate">
                                        {kb}
                                      </div>
                                    ))}
                                  </div>
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="space-y-2">
                            {cmd.examples.map((ex, idx) => (
                              <div
                                key={idx}
                                className="flex flex-col sm:flex-row sm:items-center justify-between p-2.5 rounded-xl bg-[var(--color-surface-elevated)] dark:bg-slate-950/70 border border-[var(--color-border)] dark:border-slate-800/80 gap-2"
                              >
                                <code className="font-mono text-xs text-[var(--color-primary)] dark:text-cyan-300 bg-[var(--color-primary)]/10 dark:bg-cyan-950/40 px-2.5 py-1 rounded border border-[var(--color-primary)]/20 dark:border-cyan-800/50 shrink-0">
                                  $ {ex.cmd}
                                </code>
