...
import type { BinarySearchQuizData } from './binarySearchEngine';
import './BinarySearch.css';

interface BinarySearchPredictionQuizProps {
  quizData: BinarySearchQuizData;
  onCorrectAnswer: () => void;
}

export const BinarySearchPredictionQuiz: React.FC<BinarySearchPredictionQuizProps> = ({
  quizData,
  onCorrectAnswer,
}) => {
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        >
          <HelpCircle size={15} />
          <span>BINARY SEARCH PREDICTION QUIZ</span>
        </div>
        <span
          style={{
            fontSize: '0.7rem',
      ...
      {!isAnswered ? (
        ...
        >
          ...
          >
            <span>Resume Search Execution</span>
            <ArrowRight size={14} />
          </button>
        </div>
      )}
